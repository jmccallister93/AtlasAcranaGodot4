...
func initialize(type: BuildingData.BuildingType, tile: BiomeTile):
	"""Initialize the building with type and tile data"""
	building_type = type
	target_tile = tile
	tile_position = tile.grid_position
	...
	total_production = BuildingData.get_total_production(building_type, tile.biome_type)
...
func get_base_production() -> Dictionary:
	"""Get base production without biome bonuses"""
	return base_production.duplicate()

func get_biome_bonus() -> Dictionary:
	"""Calculate the biome bonus for this building"""
	var bonus = {}
	for resource in total_production:
